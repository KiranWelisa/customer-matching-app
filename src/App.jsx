      ...
      while (iterationCount < maxIterations && !hasTimedOut && !hasError) {
        ...
        if (aiAnalysisResult) {
          ...
          if (aiSuccessCount >= 3) {
            ...
            const hasLowQualityMatch = top3AiScores.some(score => score < aiValidationThreshold);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {matches.length > 0 && (
              <div className="space-y-4">
                ...
                {/* AI Validation Threshold Slider */}
                <div className="flex flex-col items-center mt-4 gap-2">
                  <label htmlFor="ai-threshold-slider" className="text-xs text-gray-600 font-medium mb-1">AI Validation Trigger Threshold</label>
                  <input
                    ...
                    max={0.85}
                    step={0.15}
                    value={aiValidationThreshold}
                    onChange={e => setAiValidationThreshold(Number(e.target.value))}
                    className="w-56 accent-purple-600"
                  />
                  <div className="flex justify-between w-56 text-xs text-gray-500 mt-1">
                    <span className={aiValidationThreshold === 0.5 ? 'font-bold text-purple-700' : ''}>≤ 50%</span>
                    <span className={aiValidationThreshold === 0.65 ? 'font-bold text-purple-700' : ''}>≤ 65%</span>
                    <span className={aiValidationThreshold === 0.75 ? 'font-bold text-purple-700' : ''}>≤ 75%</span>
                    <span className={aiValidationThreshold === 0.85 ? 'font-bold text-purple-700' : ''}>≤ 85%</span>
                  </div>
                  <button
